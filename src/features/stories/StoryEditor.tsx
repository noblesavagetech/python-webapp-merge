...
export function StoryEditor() {
  ...
  const loadStory = async () => {
    ...
    try {
      ...
      if (storyData.chapters && storyData.chapters.length > 0) {
        ...
        await api.createChapter(storyIdNum, { title: `${storyData.title} - Chapter 1` });
  ...
  const handleAddKeyEvent = async () => {
    ...
    try {
      ...
      setKeyEvents([...keyEvents, response.event].sort((a, b) => a.order - b.order));
      setNewEventDesc('');
  ...
  const handleUpdateCharacter = async (charId: number, name: string, traits: string, backstory: string) => {
    if (!storyIdNum) return;
    try {
      await api.updateCharacter(storyIdNum, charId, { name, traits, backstory });
      const charsResponse = await api.getCharacters(storyIdNum);
      setCharacters(charsResponse.characters || []);
  ...
  const handleDeleteCharacter = async (charId: number) => {
    if (!storyIdNum || !confirm('Are you sure you want to delete this character?')) return;
    try {
  ...
  const handleUpdateBeat = async (beatId: number, description: string, order: number) => {
  ...
  const handleDeleteBeat = async (beatId: number) => {
    if (!currentChapter || !confirm('Are you sure you want to delete this beat?')) return;
    try {
  ...
  const handleUpdateWorldElement = async (elemId: number, category: string, description: string) => {
    if (!currentChapter) return;
    try {
  ...
  const handleDeleteWorldElement = async (elemId: number) => {
    if (!currentChapter || !confirm('Are you sure you want to delete this world element?')) return;
    try {
  ...
  const handleUpdateKeyEvent = async (eventId: number, description: string, order: number) => {
    if (!currentChapter) return;
    try {
  ...
  const handleDeleteKeyEvent = async (eventId: number) => {
    if (!currentChapter || !confirm('Are you sure you want to delete this key event?')) return;
    try {
  ...
  const handleGenerateProse = async () => {
    ...
    try {
      ...
      const charStr = selectedCharsProse.length > 0
      ...
      const beatInstructionsStr = beatPrompt !== defaultBeatPrompt ? `\n\nBeat Generator Instructions:\n${beatPrompt}` : '';
  ...
  const handleGenerateBeat = async () => {
    ...
    try {
      ...
      // Format character info
      const charStr = selectedCharsBeat.length > 0
      ...
      const proseInstructionsStr = prosePrompt !== defaultProsePrompt ? `\n\nProse Generator Instructions:\n${prosePrompt}` : '';
  ...
  return (
    <div className="story-editor-page">
      <div className="story-editor-container">
        ...
        <Link to="/stories" className="back-link-se">← Back to Stories</Link>
        ...
        <div className="chapter-switcher">
          <div className="chapter-tabs">
            {story.chapters?.map((chapter) => (
              <button
                key={chapter.id}
                className={`chapter-tab ${currentChapter?.id === chapter.id ? 'active' : ''}`}
                onClick={() => handleSwitchChapter(chapter.id)}
              >
                {chapter.title}
              </button>
            ))}
            <button className="chapter-tab chapter-tab-new" onClick={handleAddChapter}>
              + New Chapter
            </button>
          </div>
        </div>
        ...
        {/* Chapter Form */}
        <form className="chapter-form" onSubmit={(e) => { e.preventDefault(); handleSave(); }}>
          ...
          <button type="submit" className="btn-save">Save Chapter</button>
        </form>
              ...
              {characters.map(char => (
                <label key={char.id} className="checkbox-label">
                  <input 
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        setSelectedCharsProse([...selectedCharsProse, char.name]);
                      } else {
                        setSelectedCharsProse(selectedCharsProse.filter(n => n !== char.name));
                      }
                    }}
                  />
                  {char.name}
                  ...
                  <input 
                    ...
                    onChange={(e) => {
                      if (e.target.checked) {
                        ...
                        setSelectedCharsBeat(selectedCharsBeat.filter(n => n !== char.name));
                      }
                    }}
                  />
                  {char.name}
        ...
        <ul className="item-list">
          {beats.map(beat => (
            <li key={beat.id} className="item" style={{flexDirection: 'column', alignItems: 'stretch'}}>
              {editingBeatId === beat.id ? (
                <div style={{display: 'flex', flexDirection: 'column', gap: '0.5rem'}}>
                  ...
                  <div style={{display: 'flex', gap: '0.5rem'}}>
                    <button 
                      className="btn-save"
                      onClick={() => {
                        const order = parseInt((document.getElementById(`beat-order-${beat.id}`) as HTMLInputElement).value);
                        const desc = (document.getElementById(`beat-desc-${beat.id}`) as HTMLTextAreaElement).value;
                ...
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                  <span>{beat.order}. {beat.description}</span>
                  <div style={{display: 'flex', gap: '0.5rem'}}>
                    <button 
                      className="btn-edit"
          ...
          {beats.length === 0 && <li className="no-items">No beats/scenes yet.</li>}
        </ul>
        <div className="add-form">
          ...
          />
          <button className="btn-add" onClick={handleAddBeat}>Add Beat/Scene</button>
        </div>
        ...
        <div className="add-form">
          ...
          />
          <button className="btn-add" onClick={handleAddWorldElement}>Add Element</button>
        </div>
        <ul className="item-list">
          {worldElements.map(elem => (
            <li key={elem.id} className="item" style={{flexDirection: 'column', alignItems: 'stretch'}}>
              {editingWorldId === elem.id ? (
                <div style={{display: 'flex', flexDirection: 'column', gap: '0.5rem'}}>
                  ...
                  <div style={{display: 'flex', gap: '0.5rem'}}>
                    <button 
                      className="btn-save"
                      onClick={() => {
                        const category = (document.getElementById(`world-cat-${elem.id}`) as HTMLSelectElement).value;
                        const desc = (document.getElementById(`world-desc-${elem.id}`) as HTMLTextAreaElement).value;
                ...
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                  <span><strong>{elem.category}:</strong> {elem.description}</span>
                  <div style={{display: 'flex', gap: '0.5rem'}}>
                    <button 
                      className="btn-edit"
          ...
          {worldElements.length === 0 && <li className="no-items">No world building elements yet.</li>}
        </ul>
        ...
        <div className="add-form">
          ...
          />
          <button className="btn-add" onClick={handleAddCharacter}>Add Character</button>
        </div>
        <ul className="item-list">
          {characters.map(char => (
            <li key={char.id} className="item" style={{flexDirection: 'column', alignItems: 'stretch'}}>
              {editingCharId === char.id ? (
                <div style={{display: 'flex', flexDirection: 'column', gap: '0.5rem'}}>
                  ...
                  <div style={{display: 'flex', gap: '0.5rem'}}>
                    <button 
                      className="btn-save"
                      onClick={() => {
                        const name = (document.getElementById(`char-name-${char.id}`) as HTMLInputElement).value;
                        const traits = (document.getElementById(`char-traits-${char.id}`) as HTMLTextAreaElement).value;
                        const backstory = (document.getElementById(`char-back-${char.id}`) as HTMLTextAreaElement).value;
                ...
                <div style={{display: 'flex', flexDirection: 'column'}}>
                  ...
                  {char.traits && <div className="item-detail">Traits: {char.traits}</div>}
                  {char.backstory && <div className="item-detail">Backstory: {char.backstory}</div>}
                </div>
              )}
            </li>
          ))}
          {characters.length === 0 && <li className="no-items">No characters yet.</li>}
        </ul>
        ...
        <div className="add-form">
          ...
          />
          <button className="btn-add" onClick={handleAddKeyEvent}>Add Key Event</button>
        </div>
        <ul className="item-list">
          {keyEvents.map(event => (
            <li key={event.id} className="item" style={{flexDirection: 'column', alignItems: 'stretch'}}>
              {editingEventId === event.id ? (
                <div style={{display: 'flex', flexDirection: 'column', gap: '0.5rem'}}>
                  ...
                  <div style={{display: 'flex', gap: '0.5rem'}}>
                    <button 
                      className="btn-save"
                      onClick={() => {
                        const order = parseInt((document.getElementById(`event-order-${event.id}`) as HTMLInputElement).value);
                        const desc = (document.getElementById(`event-desc-${event.id}`) as HTMLTextAreaElement).value;
                ...
                <div style={{display: 'flex', justifyContent: 'space-between', alignItems: 'center'}}>
                  <span>{event.order}. {event.description}</span>
                  <div style={{display: 'flex', gap: '0.5rem'}}>
                    <button 
                      className="btn-edit"
          ...
          {keyEvents.length === 0 && <li className="no-items">No key events yet.</li>}
        </ul>

        <hr className="section-divider" />

        {/* AI Context Inspector - Moved to bottom as troubleshooting tool */}
        <div className="ai-generator" style={{marginBottom: '1.5rem', background: 'var(--color-bg-tertiary, #252541)', padding: '1rem', borderRadius: '0.5rem'}}>
          <h3 className="generator-title">📋 AI Context Inspector (Troubleshooting)</h3>
          <p style={{color: 'var(--color-text-secondary, #9ca3af)', marginBottom: '1rem', fontSize: '0.875rem'}}>
            This shows what data the AI generators have access to for this chapter:
          </p>
          <div style={{background: 'var(--color-bg-secondary, #1a1a2e)', padding: '1rem', borderRadius: '0.375rem', fontSize: '0.875rem', fontFamily: 'monospace', overflowX: 'auto'}}>
            <pre style={{margin: 0, whiteSpace: 'pre-wrap', wordBreak: 'break-word'}}>
{`Story: ${story.title}
Current Chapter: ${currentChapter?.title || 'None'}

Characters (${characters.length}):
${characters.map(c => `  - ${c.name}${c.traits ? `\n    Traits: ${c.traits}` : ''}${c.backstory ? `\n    Backstory: ${c.backstory}` : ''}`).join('\n') || '  (none)'}
...
${(currentChapter?.text || '(empty)').substring(0, 200)}${(currentChapter?.text || '').length > 200 ? '...' : ''}`}
            </pre>
          </div>
        </div>
      </div>
